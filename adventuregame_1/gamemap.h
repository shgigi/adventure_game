...
namespace game {
	...
	private:
		...
		std::map<int, std::unique_ptr<Room>> idMap;
		std::vector<Room*> intersections;
		void walkDirection(Directions direction, Room* room);
		void findIntersections();
		bool detectLoop();
		void createNullRoom();
		int checkNullExit(int room_id, Directions direction);
		int nullRoomId;
		int startRoomId;
	public:
		Map();
		...
		int getEdge(int current_room_id, int prev_room_id, Directions direction, Directions prev_direction, int distance);
		int getNullRoomId();
		std::vector<Room*> getRooms();
		Directions getOppositeDirection(Directions direction);
		std::string_view directionToString(Directions direction);
		//Room getEastern();
		/*
